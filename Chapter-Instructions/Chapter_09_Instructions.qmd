...
Earlier we explored making plots with ggplot2.
...
```{r color vs shape aesthetic on mpg}
#| eval: false
# Left
...
Learn more about all possible aesthetic mappings in the aesthetic specifications vignette at https://ggplot2.tidyverse.org/articles/ggplot2-specs.html.
...
For instance, the following code produces two plots that represent the same data (mpg):

```{r two types of plots}
#| eval: false
ggplot(mpg, aes(x = displ, y = hwy)) + 
  geom_point()
...
```{r overlay plot with original data}
#| eval: false
...
```{r change shape for specific layer}
#| eval: false
ggplot(mpg, aes(x = displ, y = hwy)) + 
  geom_point() + 
  geom_point(
...
```{r example plots for skew and outliers}
#| eval: false
...
![Cars engine size (displ) and cars fuel efficiency on highways in mpg package](Images/Ch9_Exercise_pt2_Q4.png){#fig-cars width=50% fig-alt="Scatterplot showing negative correlation between a car's engine size and its fuel efficiency on the highway, with values specific per car type" fig-align="center" .border}
...
1. Identify a limitation in the following plot, and show how you would address and improve it. (Hint: Explore the data in the question.)
```{r ch 9 exercises pt 5 #1}
#| eval: false
...
We can expand on the graphing template you learned in Chapter 1 by adding position adjustments, stats, coordinate systems, and faceting:
