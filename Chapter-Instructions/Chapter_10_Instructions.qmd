...
Your goal during EDA is to develop an understanding of your data. The easiest way to do this is to use questions as tools to guide your investigation. When you ask a question, the question focuses your attention on a specific part of your dataset and helps you decide which graphs, models, or transformations to make. There is no rule about which questions you should ask to guide your research. However, two types of questions will always be useful for making discoveries within your data. You can loosely word these questions as:
...
Variation is the tendency of the values of a variable to change from measurement to measurement. You can see variation easily in real life; if you measure any continuous variable twice, you will get two different results. Variables can also vary if you measure across different subjects (e.g., the eye colors of different people) or at different times (e.g., the energy levels of an electron at different moments). Every variable has its own pattern of variation, which can reveal interesting information about how it varies between measurements on the same observation as well as across observations.
...
```{r distribution of diamond weights}
ggplot(diamonds, aes(x = carat)) +
  geom_histogram(binwidth = 0.5)
```

Now that you can visualize variation, what should you look for in your plots? And what type of follow-up questions should you ask?
...
```{r distribution of small diamond weights}
smaller <- diamonds |> 
  filter(carat < 3)
...
```{r variation of y values in diamond dataset}
...
```{r zoom in to small values on y axis}
ggplot(diamonds, aes(x = y)) + 
  geom_histogram(binwidth = 0.5) +
...
```{r zoom into small values on x axis}
unusual <- diamonds |> 
  filter(y < 3 | y > 20) |> 
  select(price, x, y, z) |>
  arrange(y)
...
```{r drop whole row with unusual values}
diamonds2 <- diamonds |> 
  filter(between(y, 3, 20))
```

This option is not recommended because one invalid value doesn’t imply that all the other values for that observation are also invalid.
...
```{r unusual values to missing values}
diamonds2 <- diamonds |> 
...
```{r plot diamonds without missing values}
ggplot(diamonds2, aes(x = x, y = y)) + 
  geom_point()
```

To suppress that warning, set na.rm = TRUE:
```{r suppress warning for removed missing values}
ggplot(diamonds2, aes(x = x, y = y)) + 
...
```{r cancelled and non-cancelled flights}
...
```{r price of diamond v cut quality}
...
```{r price v diamond density boxplot}
...
cut is an ordered factor: fair is worse than good, which is worse than very good and so on. Many categorical variables don’t have such an intrinsic order, so you might want to reorder them to make a more informative display. One way to do that is with fct_reorder(). For example, take the class variable in the mpg dataset. You might be interested to know how highway mileage varies across classes:
```{r highway mileage variation across classes}
ggplot(mpg, aes(x = class, y = hwy)) +
  geom_boxplot()
```

To make the trend easier to see, we can reorder class based on the median value of hwy:
```{r reorder boxplots off median values}
...
```{r compute count with dplyr}
diamonds |> 
  count(color, cut)
```

Then visualize with geom_tile() and the fill aesthetic:
```{r diamonds tile plot}
diamonds |> 
  count(color, cut) |>  
...
```{r carat v price diamonds transparent}
ggplot(smaller, aes(x = carat, y = price)) + 
...
```{r carat v price diamonds 2d bin and hexagon plot }
ggplot(smaller, aes(x = carat, y = price)) +
  geom_bin2d()
...
```{r bin carat v price diamonds boxplot}
ggplot(smaller, aes(x = carat, y = price)) + 
...
```{r log transform and model price v carat diamond}
library(tidymodels)
...
```{r remove carat v price to focus on cut v price}
